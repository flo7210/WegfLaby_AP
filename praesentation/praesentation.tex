...
\item Eine Ecke zwischen \verb+v+ und \verb+w+ bedeutet, dass zwischen \verb+v+ und \verb+w+ keine Wand existiert
\end{itemize}
\item Knoten sind Tupel \verb+(x, y)+
\item Zu jedem Knoten \verb+v+ wird eine Liste von Knoten \verb+w+ gespeichert, so dass zwischen \verb+w+ und \verb+v+ eine Ecke existiert, eine \textit{Adjazenzliste}
\item Da die Maße des Labyrinths und die des Touchscreens bekannt sind, können wir La\-by\-rinth\-ko\-or\-di\-na\-ten in Touch\-screen\-ko\-or\-di\-na\-ten umwandeln und umgekehrt
\end{itemize}
\end{frame}
