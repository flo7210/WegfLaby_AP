...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
	...
	\node [rounded] at (-2, 0) (maze) {\verb~Maze~};
	\node [rounded] at (3, 0) (balancer) {\verb~Balancer~};
	\node [block, dashed] at (1, 1) (mcu) {MCU-Code};
	\node at (-3, 1) {Labyrinth};
	\node at (4, 1) {TS};
	\path[line] (main) -- (balancer);
	\path[line, dashed] (balancer) -- (mcu);
	\path[line] (main) -- (maze);
	\draw[line, <->] (-2.3, .3) -- (-2.8, .8);
	\draw[line, <->] (3.3, .3) -- (3.8, .8);
  \end{tikzpicture}
\end{figure}
\end{frame}
...
\begin{frame}[fragile,t]{Ergebnis}{Die \verb~Maze~-Klasse}
Text.
\end{frame}

\begin{frame}[fragile,t]{Ergebnis}{Die \verb~Balancer~-Klasse: Kommunikation zwischen PC und MCU}
Text.
\end{frame}

\begin{frame}[fragile,t]{Ergebnis}{Ablauf der Wanderkennung}
\blue{Flowchart zu \texttt{detect\textunderscore walls(anchor, maze, dualmaze)}:}
\begin{small}
\begin{figure}[h]
  \centering
  \begin{tikzpicture}[
	...
	\node at (0, -5) [kugel] (kugel) {Kugel ist ausbalanciert};
